...
class Schedule extends React.Component {
  ...
  render() {
    return <div className={styles.schedule}>
      ...
      {this.state.rows.map((row, i) => {
        console.log(`ScheduleRow ${this.state.weeks}`)
        return <ScheduleRow
          title={row.title}
  ...
  componentDidUpdate(prevProps) {
    ...
    if (prevProps.startDate !== this.props.startDate) {
      weeks = this.generateWeeks(this.props.startDate, prevProps.endDate)
      this.setState({
        grid: {...this.state.grid, 'startDate': this.props.startDate, 'width': prevProps.dayWidth * weeks.length * 7 },
        weeks: weeks
      });
    }

    if (prevProps.endDate !== this.props.endDate) {
      weeks = this.generateWeeks(prevProps.startDate, this.props.endDate)
      this.setState({
        grid: {...this.state.grid, endDate: this.props.endDate, 'width': prevProps.dayWidth * weeks.length * 7},
        weeks: weeks
      });
    }
